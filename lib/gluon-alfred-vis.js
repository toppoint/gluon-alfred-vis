      var siteUrl='http://freifunk.in-kiel.de'
	  var ffmapPath='/ffmap'
      var geomapUrl=siteUrl+ffmapPath+'/geomap.html'
      ...
      function showResults () {
        ...
        for (var e in arr) {
          html += '<tr>'
            +'<td><a target="_blank" href="http://['+arr[e].address+']/">'+htmlentities(arr[e].hostname)+'</a></td>'
            ...
            +'<td>'+(arr[e].firmware_base === undefined ? '' : arr[e].firmware_base)+' '+(arr[e].firmware_release === undefined ? '' : arr[e].firmware_release)+'</td>'
            +'<td>'+(arr[e].model === undefined ? '' : arr[e].model)+'</td>'
            +'<td>'+(arr[e].autoupdate === undefined ? '' : arr[e].autoupdate)+'</td>'
            +'<td>'+(arr[e].mac === undefined ? '' : '<a href="'+siteUrl+ffmapPath+'/nodeplot.cgi?'+arr[e].mac+'" target="_blank">'+arr[e].mac+'</a>')+'</td>'
            +'<td class="contact-row">'+htmlentities(arr[e].contact)+'</td>'
            +'<td>'+((arr[e].longitude === undefined || arr[e].latitude === undefined) ? '' : '<a href="'+geomapUrl+'?lat='+arr[e].latitude+'&lon='+arr[e].longitude+'" target="_blank">'+arr[e].longitude.toFixed(5)+' '+arr[e].latitude.toFixed(5)+'</a>' )+'</td>'
            +'</tr>';
        }
        $('#results').html(html)
          .append(arr.length+' Knoten');
      }
      ...
      function reload() {
        $.getJSON('alfred-158.json', function(data) {
          $.each( data, function( key,val ) {
            ...
            try { nodes[key].firmware_base = val['software']['firmware']['base'];                                   } catch (e) {};
            try { nodes[key].firmware_release = val['software']['firmware']['release'];                             } catch (e) {};
